...
import { collection, addDoc, getDocs, updateDoc, doc, increment, query, orderBy, getDoc, arrayUnion } from "firebase/firestore";
...
export async function upvoteReport(reportId, userId) {
    if (!userId) {
        // Only authenticated users can upvote; caller already guards this.
        return;
    }
    try {
        ...
        if (upvotedBy.includes(userId)) {
            // User has already upvoted this report; do nothing.
            return;
        }
